...
const initiallyDisplayedCurrencies = ["EUR", "CZK", "USD"];
let baseCurrency;
let baseCurrencyAmount;

var favorite_currencies = localStorage.getItem('favorite_currencies');
favorite_currencies = [];

if(favorite_currencies.indexOf(initiallyDisplayedCurrencies) === -1) {
    localStorage.setItem('favorite_currencies', (initiallyDisplayedCurrencies));
    favorite_currencies = localStorage.getItem('favorite_currencies');
} else {
    var favorite_currencies = localStorage.getItem('favorite_currencies');
}
...
function newCurrenciesListItem(currency) {
  ...
  const exchangeRate =
    currency.abbreviation === baseCurrency
      ? 1
      : (currency.rate / baseCurrencyRate).toFixed(3);
  ...
  currenciesList.insertAdjacentHTML(
    ...
    <div class="info">
        <p class="input"><span class="currency-symbol">${
          currency.symbol
        }</span><input placeholder="${exchangeRate}" value="${inputValue}" type="number"</p>
